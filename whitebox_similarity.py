...
def predict(model, torch_stft, sound, device):
    ...
    out, output_sizes = model(spec, input_sizes)
    return out

def pgd_attack(audio_path, output_file, model_path, split_index, dataset_path, epsilon, alpha, mode, PGD_iter):
    device = 'cuda'
    sample_rate = 16000
    ...
    attacker = Attacker(model=model, sound=sound, target=target.upper(), decoder=decoder, device=device, save=output_file)

    return attacker.attack(epsilon = epsilon, alpha=float(alpha), attack_type=mode, PGD_round=PGD_iter, attack_range=[verb_range.start, verb_range.stop])
